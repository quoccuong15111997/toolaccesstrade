...
public class TopSellActivity extends AppCompatActivity {
    ...
    private void callApiTopSell(String dateFrom, String dateTo, String merchant, final ArrayList<TopSellAPI> arrTopSell){
        TopSellService.getInstance().getTopSell(new Callback<ArrayList<TopSellAPI>>() {
            @Override
            public void onResponse(Call<ArrayList<TopSellAPI>> call, Response<ArrayList<TopSellAPI>> response) {
                if (response.isSuccessful()){
                    ArrayList<TopSellAPI> topSellAPIS=response.body();
                    Toast.makeText(TopSellActivity.this, topSellAPIS.get(1).getName(), Toast.LENGTH_SHORT).show();
                    arrTopSell.add(topSellAPIS.get(1));
                }
            }

            @Override
            public void onFailure(Call<ArrayList<TopSellAPI>> call, Throwable t) {

            }
        },dateFrom,dateTo,merchant);
    }
